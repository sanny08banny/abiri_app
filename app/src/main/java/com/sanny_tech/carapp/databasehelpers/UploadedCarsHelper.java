...
public class UploadedCarsHelper extends SQLiteOpenHelper {
    ...
    private static final String COLUMN_UNAVAILABLE_DATES = "unavailable_dates";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // Handle schema upgrades here when needed
    }
    ...
    public long insertCar(Car car) {
        if (getCarById(car.getCar_id()) == null) {
            ...
            values.put(COLUMN_UNAVAILABLE_DATES, TextUtils.join(",", car.getUnavailable_dates()));
        ...
        if (cursor != null) {
            if (cursor.moveToFirst()) {
                do {
                    ...
                    String unavailableCsv = cursor.getString(6);
                    if (unavailableCsv != null && !unavailableCsv.isEmpty()) {
                        car.setUnavailable_dates(new ArrayList<>(Arrays.asList(unavailableCsv.split(","))));
                    } else {
                        car.setUnavailable_dates(new ArrayList<>());
                    }
                    ...
                    if (imageString != null && !imageString.isEmpty()) {
                        car.setCar_images(new ArrayList<>(Arrays.asList(imageString.split(","))));
                    } else {
                        car.setCar_images(new ArrayList<>());
                    }
    ...
    public void saveCarToFirestore(Car car) {
        ...
        carMap.put("unavailable_dates", car.getUnavailable_dates());
        carMap.put("amount", car.getDaily_amount());
        carMap.put("downpayment", car.getDaily_downpayment_amt());
        carMap.put("car_images", car.getCar_images());
    ...
    public String getAvailabilityStatus(Car car) {
        // Format today's date as "yyyy-MM-dd"
        String today = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(new Date());

        List<String> unavailable = car.getUnavailable_dates();
        if (unavailable != null && unavailable.contains(today)) {
            return "Unavailable";
        }
        return "Available";
    }
